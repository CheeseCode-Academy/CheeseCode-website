// ============================================
// ⚡ موتور پایتون | CodaX Academy
// مدیریت کش و نمونه مفسر پایتون
// ============================================

window.pyodideInstance = null;
window.isPyodideLoading = false;

const LIBS_CACHE_NAME = 'codax-py-libs-v1';
...
async function initPythonEngine() {
    ...
    console.log("⏳ در حال بارگذاری مفسر پایتون...");
    ...
    try {
        ...
        try {
            const libsCache = await caches.open(LIBS_CACHE_NAME);
            for (let lib of AVAILABLE_LIBS) {
                try {
                    let cachedResponse = await libsCache.match(lib.url);
                    if (cachedResponse) {
            ...
            console.warn('⚠️ Cache API در دسترس نیست:', e);
        }

        console.log("✨ پایتون با موفقیت آماده شد!");
    } catch (err) {
